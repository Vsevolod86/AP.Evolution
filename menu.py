...
class Menu(Screen):
    ...
    def position_cursor_up(self):
        self.position_cursor = (
            ...
            else self.num_header - 1
        )

    def position_cursor_down(self):
        self.position_cursor = (
            self.position_cursor + 1
            if self.position_cursor != self.num_header - 1
            else 0
        )
    ...
    def process_entities(self):
        ...
        for i, header in enumerate(self.list_header):
            self.draw_text(
                ...
                20,
                250,
                (1 + i) * (int(self.surface.get_height() / (self.num_header + 1))),
            )
    ...
    def draw_cursor(self):
        self.draw_text(
            ...
            25,
            100,
            (1 + self.position_cursor)
            * (int(self.surface.get_height() / (self.num_header + 1))),
        )
